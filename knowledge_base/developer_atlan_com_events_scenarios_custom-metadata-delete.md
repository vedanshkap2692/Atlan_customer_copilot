# Events when custom metadata is removed from an asset¶
(source: https://developer.atlan.com/events/scenarios/custom-metadata-delete/)
...
EventsEventsEvent triggersEvent triggersAsset is createdAsset is updatedAsset is deletedCustom metadata is addedCustom metadata is removedCustom metadata is removedTable of contentsBUSINESS_ATTRIBUTE_UPDATEAsset is taggedAsset is untaggedLineage is createdEvent typesEvent typesENTITY_CREATEENTITY_UPDATEENTITY_DELETEBUSINESS_ATTRIBUTE_UPDATECLASSIFICATION_ADDCLASSIFICATION_DELETE

Event triggersEvent triggersAsset is createdAsset is updatedAsset is deletedCustom metadata is addedCustom metadata is removedCustom metadata is removedTable of contentsBUSINESS_ATTRIBUTE_UPDATEAsset is taggedAsset is untaggedLineage is created
...
Custom metadata is removedCustom metadata is removedTable of contentsBUSINESS_ATTRIBUTE_UPDATE
...
When custom metadata is removed from anassetin Atlan, the following event is emitted.
...
Atlan always emits aBUSINESS_ATTRIBUTE_UPDATEevent when custom metadata is removed from an asset.

BUSINESS_ATTRIBUTE_UPDATE

Thedetail.message.mutatedDetailsobject contains the details of the removed attributes. Any attributes with a value ofnullwere removed.
