# Manage Kafka assets¶
(source: https://developer.atlan.com/patterns/create/kafka/)
...
Manage Kafka assetsManage Kafka assetsTable of contentsAsset structureConnectionKafkaTopicKafkaConsumerGroupAvailable relationshipsAtlasGlossaryTermLinkReadmeProcess

Asset structureConnectionKafkaTopicKafkaConsumerGroup
...
A Kafkaconnectionrequires anameandqualifiedName. For creation, specific settings are also required to distinguish it as a Kafka connection rather than another type of connection. In addition,at least oneofadminRoles,adminGroups, oradminUsersmust be provided.
...
Set the type of connection toKAFKA.
...
{"entities":[{"typeName":"Connection",// (1)"attributes":{"name":"kafka-connection",// (2)"connectorName":"kafka",// (3)"qualifiedName":"default/kafka/123456789",// (4)"category":"eventbus",// (5)"adminRoles":[// (6)"e7ae0295-c60a-469a-bd2c-fb903943aa02"],"adminGroups":[// (7)"group2"],"adminUsers":[// (8)"jsmith"]}}]}
...
TheconnectorNamemust be exactlykafka.

connectorName

kafka

ThequalifiedNameshould follow the pattern:default/kafka/<epoch>, where<epoch>is the time in milliseconds at which the connection is being created.
...
AKafkaTopicrequires anameand aqualifiedName.
For creation, you also need to specify theconnectionQualifiedNameof the connection for the topic.
...
KafkaTopickafkaTopic=KafkaTopic.creator(// (1)"myKafkaTopic",// (2)connectionQualifiedName)// (3).build();response=kafkaTopic.save(client);// (4)kafkaTopic=response.getResult(kafkaTopic);// (5)
...
Provide thequalifiedNameof the Kafka connection.

qualifiedName

Actually call Atlan to create the topic. Because this operation will persist the asset in Atlan, you mustprovide it anAtlanClientthrough which to connect to the tenant.
...
kafka_topic=KafkaTopic.creator(# (1)name="myKafkaTopic",# (2)connection_qualified_name=connection_qualified_name# (3))response=client.asset.save(kafka_topic)# (4)kafka_topic_qualifed_name=response.assets_created(asset_type=KafkaTopic)[0].qualified_name# (5)
...
Actually call Atlan to create the topic.
...
varkafkaTopic=KafkaTopic.creator(// (1)"myKafkaTopic",// (2)connectionQualifiedName)// (3).build()response=kafkaTopic.save(client)// (4)kafkaTopic=response.getResult(kafkaTopic)// (5)
...
{"entities":[{"typeName":"KafkaTopic",// (1)"attributes":{"name":"myKafkaTopic",// (2)"qualifiedName":"default/kafka/123456789/topic/myKafkaTopic",// (3)"connectionQualifiedName":"default/kafka/123456789",// (4)"connectorName":"kafka"// (5)}}]}

{"entities":[{"typeName":"KafkaTopic",// (1)"attributes":{"name":"myKafkaTopic",// (2)"qualifiedName":"default/kafka/123456789/topic/myKafkaTopic",// (3)"connectionQualifiedName":"default/kafka/123456789",// (4)"connectorName":"kafka"// (5)}}]}

ThetypeNamemust be exactlyKafkaTopic.
...
ThequalifiedNameshould follow the pattern:default/kafka/<epoch>/topic/<name>,
wheredefault/kafka/<epoch>is thequalifiedNameof the connection
for this topic and<name>is the unique name for this topic.

qualifiedName

default/kafka/<epoch>/topic/<name>
...
TheconnectionQualifiedNamemust be the exactqualifiedNameof the connection for this topic.
...
AKafkaConsumerGrouprequires anameand aqualifiedName. For creation, you also need to
specify the list ofkafkaTopicQualifiedNamesof the topics that will contain the consumer group.
...
KafkaConsumerGroupconsumerGroup=KafkaConsumerGroup.creatorObj(// (1)"myKafkaConsumerGroup",// (2)List.of(kafkaTopic))// (3).build();response=consumerGroup.save(client);// (4)
...
consumer_group=KafkaConsumerGroup.creator(# (1)name="myKafkaConsumerGroup",# (2)kafka_topic_qualified_names=[kafka_topic_qualified_name],# (3))response=client.asset.save(consumer_group)# (4)
...
Provide the list ofqualified_namesof the topic for this consumer group.
...
valconsumerGroup=KafkaConsumerGroup.creatorObj(// (1)"myKafkaConsumerGroup",// (2)listOf(kafkaTopic))// (3).build()response=consumerGroup.save(client)// (4)
...
ThequalifiedNameshould follow the pattern:default/kafka/<epoch>/consumer-group/<name>,
wheredefault/kafka/<epoch>is the qualifiedName of the connection that contains
this consumer group and<name>is the unique name for this consumer group.

qualifiedName

default/kafka/<epoch>/consumer-group/<name>
...
The list of topics in which this consumer group exists is embedded in thekafkaTopicsattribute.

kafkaTopics

ThetypeNamefor this embedded reference must beKafkaTopic.

typeName

KafkaTopic

To complete the reference, you must include auniqueAttributeswith thequalifiedNameof the topic. Note: the topic must already exist 
...
The list of topic qualified names.
...
Every level of theKafkastructure is anAsset,
