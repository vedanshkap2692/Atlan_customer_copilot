# Full text queries¶
(source: https://developer.atlan.com/search/queries/text/)
...
Full text queries allow you to find results based on analyzed text fields.1For example, by a word or phrase within a longer description.

Unliketerm-level queries, the search terms you use in a full-text queryareanalyzed. This means what you search for is tokenized first (broken up into separate words), singular / plural variants determined, synonyms applied, and so on.
...
Match queriesreturn results where the asset's value for that attribute matches some part of what you're searching.

123

IndexSearchRequestindex=client.assets.select()// (1).where(Asset.NAME.match("tmp"))// (1).toRequest();
...
Chain awhere()onto the select, with the static constant representing a field of the type you want to search to start a query, in this case theNAMEof anAsset. Adding thematch()predicate creates a match query.Equivalent query through ElasticQuerybyMatch=MatchQuery.of(m->m.field("name").query("tmp")._toQuery();

Chain awhere()onto the select, with the static constant representing a field of the type you want to search to start a query, in this case theNAMEof anAsset. Adding thematch()predicate creates a match query.
...
QuerybyMatch=MatchQuery.of(m->m.field("name").query("tmp")._toQuery();

QuerybyMatch=MatchQuery.of(m->m.field("name").query("tmp")._toQuery();

123456

frompyatlan.model.assetsimportAssetfrompyatlan.model.fluent_searchimportFluentSearchindex=(FluentSearch()# (1).where(Asset.NAME.match("tmp"))# (2)).to_request()
...
Use the class variable representing a field of the type you want to search to start a query, in this case theNAMEof anAsset. Adding thematch()predicate creates a match query.
...
valindex=client.assets.select()// (1).where(Asset.NAME.match("tmp"))// (1).toRequest()
...
Chain awhere()onto the select, with the static constant representing a field of the type you want to search to start a query, in this case theNAMEof anAsset. Adding thematch()predicate creates a match query.Equivalent query through ElasticvalbyMatch=MatchQuery.of(m->m.field("name").query("tmp")._toQuery()
...
valbyMatch=MatchQuery.of(m->m.field("name").query("tmp")._toQuery()

valbyMatch=MatchQuery.of(m->m.field("name").query("tmp")._toQuery()

1234567891011

{"dsl":{// (1)"query":{// (2)"match":{// (3)"name":{// (4)"query":"tmp"// (5)}}}}}
...
For a match query, there needs to be amatchobject embedded within thequeryobject.
...
The value for this field (attribute) to match against should be given through thequeryproperty.
...
Unlike a regularmatchquery, which performs a full-text search and may return results with individual words appearing in any order, amatch_phrasequery ensures that the terms appear in the exact order specified.

match

match_phrase

For example, to search for assets where thedescriptionfield contains the exact phrase"data pipeline":
...
frompyatlan.model.assetsimportAssetfrompyatlan.model.fluent_searchimportFluentSearchindex=(FluentSearch()# (1).where(Asset.DESCRIPTION.match_phrase("data pipeline"))# (2)).to_request()
...
Use the class variable representing a field of the type you want to search to start a query, in this case theDESCRIPTIONof anAsset. Adding thematch_phrase()predicate creates a match phrase query.
...
{"dsl":{// (1)"query":{// (2)"match_phrase":{// (3)"description":{// (4)"query":"data pipeline"// (5)}}}}}
...
For a match phrase query, there needs to be amatch_phraseobject embedded within thequeryobject.
...
This page is a summary of the details in the Elasticsearch Guide'sFull text queries↩
