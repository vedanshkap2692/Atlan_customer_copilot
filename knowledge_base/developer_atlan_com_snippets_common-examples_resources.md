...
(source: https://developer.atlan.com/snippets/common-examples/resources/)
...
Add asset resourcesAdd asset resourcesTable of contentsAdd to an existing assetRemove from an existing asset
...
Resources (links) can only be added toassetsafter an asset exists. (The asset itself must be created first.)
...
Tabletable=Table.refByQualifiedName("default/snowflake/1234567890/reln_db/reln_schema/customers");// (1)Linklink=Link.creator(// (2)table,// (3)"Definition",// (4)"https://en.wikipedia.org/wiki/Customer")// (5).build();AssetMutationResponseresponse=link.save(client);// (6)assertresponse.getCreatedAssets().size()==1// (7)assertresponse.getUpdatedAssets().size()==1// (8)
...
Use thecreator()method to initialize the link with allnecessary attributes for creating it.
...
Call thesave()method to actually create the link and attach it to the asset. Because this operation will persist the asset in Atlan, you mustprovide it anAtlanClientthrough which to connect to the tenant.
...
Use thecreate()method to build a link with allnecessary attributes for creating it.
...
Call thesave()method to actually create the link and attach it to the asset.
...
valtable=Table.refByQualifiedName("default/snowflake/1234567890/reln_db/reln_schema/customers")// (1)vallink=Link.creator(// (2)table,// (3)"Definition",// (4)"https://en.wikipedia.org/wiki/Customer")// (5).build()valresponse=link.save(client)// (6)assert(response.createdAssets.size==1)// (7)assert(response.updatedAssets.size==1)// (8)
...
{"entities":[// (1){"typeName":"Link",// (2)"attributes":{"name":"Definition",// (3)"qualifiedName":"540182b5-f47b-4ba1-a247-6ca9ffd9f37a",// (4)"link":"https://en.wikipedia.org/wiki/Customer",// (5)"asset":[// (6){"typeName":"Table","uniqueAttributes":{"qualifiedName":"default/snowflake/1234567890/reln_db/reln_schema/customers"}}]}}]}
...
You must provide the exact type name for the link asset, which will always beLink(case-sensitive).

Link

You must also provide a name for the link. This will show up on the UI as the title of the link.

You must also provide a uniquequalifiedNamefor the link. This will not show up on the UI, and must be a unique UUID (generated yourself).

qualifiedName

The URL for the link should be provided in thelinkfield.
