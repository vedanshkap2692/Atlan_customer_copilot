...
(source: https://developer.atlan.com/snippets/users-groups/delete/)
...
Users and groupsUsers and groupsCreate users and groupsRetrieve users and groupsUpdate users and groupsDelete users and groupsDelete users and groupsTable of contentsDelete a groupDelete a userManage SSO group mapping
...
Delete users and groupsDelete users and groupsTable of contentsDelete a groupDelete a user
...
Deleting users and groups uses a similar pattern to the retrieval operations. For this you can use static methods provided by theAtlanUserandAtlanGroupclasses.
...
To delete a group, you must specify its GUID and can simply call theAtlanGroup.delete()method. Because this operation will remove the group from Atlan, you mustprovide it anAtlanClientthrough which to connect to the tenant.
...
frompyatlan.client.atlanimportAtlanClientclient=AtlanClient()client.group.purge("e79cb8eb-2bb6-4821-914c-f8dfd21fedc7")# (1)

frompyatlan.client.atlanimportAtlanClientclient=AtlanClient()client.group.purge("e79cb8eb-2bb6-4821-914c-f8dfd21fedc7")# (1)

To delete a group, you must specify its GUID and can simply call thegroup.purge()method.
...
To delete a group, you must specify its GUID and can simply call theGroupClient.Purge()method.
...
All details are in the URL itself.Group ID in the URLNote that you must provide the unique ID (GUID) of the group to delete it.
...
response,atlanErr:=ctx.UserClient.RemoveUser("test-user-1","test-user-2","test-user-3",)// (1)ifatlanErr!=nil{logger.Log.Errorf("Error deleting user: %v",atlanErr)// (2)}

response,atlanErr:=ctx.UserClient.RemoveUser("test-user-1","test-user-2","test-user-3",)// (1)ifatlanErr!=nil{logger.Log.Errorf("Error deleting user: %v",atlanErr)// (2)}

Call thectx.UserClient.RemoveUser()method with the following parameters :userName: The username of the user to be removed ("test-user-1"in this example).transferToUserName: The username of the user to whom the assets should be transferred ("test-user-2"in this example).wfCreatorUserName (optional): The username of the workflow creator ("test-user-3"in this example). Ifnil, it defaults totransferToUserName.

Call thectx.UserClient.RemoveUser()method with the following parameters :
...
wfCreatorUserName (optional): The username of the workflow creator ("test-user-3"in this example). Ifnil, it defaults totransferToUserName.
