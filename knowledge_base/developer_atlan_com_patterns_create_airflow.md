...
(source: https://developer.atlan.com/patterns/create/airflow/)
...
Manage Airflow assetsManage Airflow assetsTable of contentsAsset structureConnectionAirflowDagAirflowTaskAvailable relationshipsAtlasGlossaryTermLinkReadmeProcess

Asset structureConnectionAirflowDagAirflowTask
...
An Airflowconnectionrequires anameandqualifiedName. For creation, specific settings are also required to distinguish it as an Airflow connection rather than another type of connection. In addition,at least oneofadminRoles,adminGroups, oradminUsersmust be provided.
...
Set the type of connection toAIRFLOW.
...
valadminRoleGuid=client.roleCache.getIdForName("\$admin")// (1)valconnection=Connection.creator(// (2)"airflow-connection",// (3)AtlanConnectorType.AIRFLOW,// (4)listOf(adminRoleGuid),// (5)listOf("group2"),// (6)listOf("jsmith"))// (7).build()valresponse=connection.save(client)// (8)valconnectionQualifiedName=response.createdAssets[0].qualifiedName// (9)
...
{"entities":[{"typeName":"Connection",// (1)"attributes":{"name":"airflow-connection",// (2)"connectorName":"airflow",// (3)"qualifiedName":"default/airflow/123456789",// (4)"category":"elt",// (5)"adminRoles":[// (6)"e7ae0295-c60a-469a-bd2c-fb903943aa02"],"adminGroups":[// (7)"group2"],"adminUsers":[// (8)"jsmith"]}}]}
...
TheconnectorNamemust be exactlyairflow.

connectorName

airflow

ThequalifiedNameshould follow the pattern:default/airflow/<epoch>, where<epoch>is the time in milliseconds at which the connection is being created.
...
AnAirflowDagrequires anameand aqualifiedName.
For creation, you also need to specify theconnectionQualifiedNameof the connection for the dag.
...
AirflowDagairflowDag=AirflowDag.creator(// (1)"myAirflowDag",// (2)connectionQualifiedName// (3)).build();AssetMutationResponseresponse=airflowDag.save(client);// (4)airflowDag=response.getResult(airflowDag);// (5)
...
Provide thequalifiedNameof the Airflow connection.

qualifiedName

Actually call Atlan to create the dag. Because this operation will persist the asset in Atlan, you mustprovide it anAtlanClientthrough which to connect to the tenant.

AtlanClient

Retrieve the created dag for use in subsequent creation calls. (You'd probably want to do somenullchecking first.)

null

171819202122

airflow_dag=AirflowDag.creator(# (1)name="myAirflowDag",# (2)connection_qualified_name=connection_qualified_name# (3))response=client.asset.save(airflow_dag)# (4)airflow_dag_qualifed_name=response.assets_created(asset_type=AirflowDag)[0].qualified_name# (5)
...
varairflowDag=AirflowDag.creator(// (1)"myAirflowDag",// (2)connectionQualifiedName// (3)).build()valresponse=airflowDag.save(client)// (4)airflowDag=response.getResult(airflowDag)// (5)
...
{"entities":[{"typeName":"AirflowDag",// (1)"attributes":{"name":"myAirflowDag",// (2)"qualifiedName":"default/airflow/123456789/myAirflowDag",// (3)"connectionQualifiedName":"default/airflow/123456789",// (4)"connectorName":"airflow"// (5)}}]}

{"entities":[{"typeName":"AirflowDag",// (1)"attributes":{"name":"myAirflowDag",// (2)"qualifiedName":"default/airflow/123456789/myAirflowDag",// (3)"connectionQualifiedName":"default/airflow/123456789",// (4)"connectorName":"airflow"// (5)}}]}

ThetypeNamemust be exactlyAirflowDag.
...
ThequalifiedNameshould follow the pattern:default/airflow/<epoch>/<name>,
wheredefault/airflow/<epoch>is the qualifiedName of the connection for this
...
TheconnectionQualifiedNamemust be the exactqualifiedNameof the connection for this dag.
...
AnAirflowTaskrequires anameand aqualifiedName.
For creation, you also need to specify theairflowDagQualifiedNameof the dag that will contain the task.
...
AirflowTaskairflowTask=AirflowTask.creator(// (1)"myAirflowTask",// (2)airflowDag// (3)).build();AssetMutationResponseresponse=airflowTask.save(client);// (4)
...
Provide the dag for this task. If you did not already have the DAG, you could also useAirflowDag.refByGuid()with the GUID of the DAG, orAirflowDag.refByQualifiedName()with thequalifiedNameof the DAG.
...
Actually call Atlan to create the task. Because this operation will persist the asset in Atlan, you mustprovide it anAtlanClientthrough which to connect to the tenant.

AtlanClient

2324252627

airflowTask=AirflowTask.creator(# (1)name="myAirflowTask",# (2)airflow_dag_qualified_name=airflow_dag_qualifed_name# (3))response=client.asset.save(airflowTask)# (4)
...
Provide thequalified_nameof the dag for this task.
...
valairflowTask=AirflowTask.creator(// (1)"myAirflowTask",// (2)airflowDag// (3)).build()valresponse=airflowTask.save(client)// (4)
...
ThequalifiedNameshould follow the pattern:default/airflow/<epoch>/<dag>/<name>,
wheredefault/airflow/<epoch>/<dag>is the qualifiedName of the dag that contains
...
TheconnectionQualifiedNamemust be the exact qualifiedName of the connection for this task.
...
The dag in which this task exists is embedded in theairflowDagattribute.

airflowDag

ThetypeNamefor this embedded reference must beAirflowDag.

typeName

AirflowDag

To complete the reference, you must include auniqueAttributestask
with thequalifiedNameof the dag. Note: the dag must already exist 
...
TheairflowDagNameshould be the human-readable name of the dag.

airflowDagName

TheairflowDagQualifiedNameshould be the qualifiedName of the dag.
...
Every level of theAirflowstructure is anAsset, and can therefore be related to the following other assets.
