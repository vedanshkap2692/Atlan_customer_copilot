# Events when an asset is created¶
(source: https://developer.atlan.com/events/scenarios/asset-create/)
...
EventsEventsEvent triggersEvent triggersAsset is createdAsset is createdTable of contentsENTITY_CREATEENTITY_UPDATEAsset is updatedAsset is deletedCustom metadata is addedCustom metadata is removedAsset is taggedAsset is untaggedLineage is createdEvent typesEvent typesENTITY_CREATEENTITY_UPDATEENTITY_DELETEBUSINESS_ATTRIBUTE_UPDATECLASSIFICATION_ADDCLASSIFICATION_DELETE

Event triggersEvent triggersAsset is createdAsset is createdTable of contentsENTITY_CREATEENTITY_UPDATEAsset is updatedAsset is deletedCustom metadata is addedCustom metadata is removedAsset is taggedAsset is untaggedLineage is created

Asset is createdAsset is createdTable of contentsENTITY_CREATEENTITY_UPDATE
...
When anassetis created in Atlan, the following events can be emitted.
...
Atlan always emits anENTITY_CREATEevent when an asset is created. The details in the event are for the asset that was created.
...
When a relationship is defined as part of asset creation, Atlan will also emit anENTITY_UPDATEevent. The details in this event cover the related asset.
...
AnENTITY_UPDATEevent will be emitted for the table in which that column was created.

ENTITY_UPDATE

Details of the relationship are not included indetail.message.mutatedDetails

detail.message.mutatedDetails

In the case of a parent-child relationship, the details of the relationship are not included in theENTITY_UPDATEevent's payload.
