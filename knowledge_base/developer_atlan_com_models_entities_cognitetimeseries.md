# CogniteTimeSeries¶
(source: https://developer.atlan.com/models/entities/cognitetimeseries/)
...
CogniteTimeseriesCogniteTimeseriesTable of __ (AnomaloCheck)application (Application)applicationField (ApplicationField)meanings (AtlasGlossaryTerm)dataContractLatest (DataContract)dataContractLatestCertified (DataContract)files (File)inputPortDataProducts (DataProduct)links (Link)mcIncidents (MCIncident)mcMonitors (MCMonitor)metrics (Metric)outputPortDataProducts (DataProduct)readme (Readme)schemaRegistrySubjects (SchemaRegistrySubject)sodaChecks (SodaCheck)userDefRelationshipFrom (Referenceable)userDefRelationshipTo (Referenceable)inputToAirflowTasks (AirflowTask)inputToProcesses (Process)inputToSparkJobs (SparkJob)modelImplementedAttributes (ModelAttribute)modelImplementedEntities (ModelEntity)outputFromAirflowTasks (AirflowTask)outputFromProcesses (Process)outputFromSparkJobs (SparkJob)cogniteAsset (CogniteAsset)
...
This is a complete reference for theCogniteTimeSeriesobject in Atlan, showing every possible property and relationship that can exist for these objects. For an introduction, you probably want to start with:
...
CogniteTimeSeriesinherits its attributes and relationships from these other types:
...
These attributes are inherited fromCogniteTimeSeries's supertypes (shown above):
...
These relationships are inherited fromCogniteTimeSeries's supertypes:
...
These relationships are specific to instances ofCogniteTimeSeries(and all of its subtypes).
