...
(source: https://developer.atlan.com/search/limit/)
...
Limiting detailsLimiting detailsTable of contentsIncluding asset propertiesIncluding relationship properties
...
By default, each search result will contain very limited details — typically only the unique identities (GUID andqualifiedName) andnameof each asset. This is intentional, to prevent retrieving more information than you need, as every piece of information you retrieve will add a little bit of time to the overall runtime of your request.
...
client.assets.select()// (1).includeOnResults(Table.CREATE_TIME)// (2).includeOnResults(Table.UPDATE_TIME).includeOnResults(Table.COLUMNS).stream()// (3)...

client.assets.select()// (1).includeOnResults(Table.CREATE_TIME)// (2).includeOnResults(Table.UPDATE_TIME).includeOnResults(Table.COLUMNS).stream()// (3)...

Start building a FluentSearch query (in this example from a client, using its 'assets' member'sselect()method).

select()

Chain as manyincludeOnResultscalls as you like. Each method call should include a searchable Atlan field you want to include on each search result. (And this can include relationship attributes as well, such as the columns in a table in this example.)
...
frompyatlan.client.atlanimportAtlanClientfrompyatlan.model.assetsimportTablefrompyatlan.model.fluent_searchimportFluentSearchclient=AtlanClient()request=(FluentSearch.select()# (1).include_on_results(Table.CREATE_TIME)# (2).include_on_results(Table.UPDATE_TIME).include_on_results(Table.COLUMNS)).to_request()# (3)results=client.asset.search(criteria=request)

frompyatlan.client.atlanimportAtlanClientfrompyatlan.model.assetsimportTablefrompyatlan.model.fluent_searchimportFluentSearchclient=AtlanClient()request=(FluentSearch.select()# (1).include_on_results(Table.CREATE_TIME)# (2).include_on_results(Table.UPDATE_TIME).include_on_results(Table.COLUMNS)).to_request()# (3)results=client.asset.search(criteria=request)

Start building a FluentSearch query (in this example using itsselect()method directly).

select()

Chain as manyinclude_on_resultscalls as you like. Each method call should include a searchable Atlan field you want to include on each search result. (And this can include relationship attributes as well, such as the columns in a table in this example.)
...
{"dsl":{// (1)"query":{...},"from":0,"size":100,"track_total_hits":true},"attributes":[// (2)"createTime","updateTime","columns"],"suppressLogs":true,"showSearchScore":false,"excludeMeanings":false,"excludeClassifications":false}

{"dsl":{// (1)"query":{...},"from":0,"size":100,"track_total_hits":true},"attributes":[// (2)"createTime","updateTime","columns"],"suppressLogs":true,"showSearchScore":false,"excludeMeanings":false,"excludeClassifications":false}

Define the search query as you normally would.

Specify the list of attributes you want to include on each result in theattributeslist of the request.
...
You can also specify which attributes you want to include on eachrelationshipin each search result:

12345

client.assets.select()// (1).includeOnResults(Table.COLUMNS)// (2).includeOnRelations(Column.NAME)// (3).stream()// (4)...
...
Chain as manyincludeOnResultscalls as you like, to include one or more relationships on each search result.

includeOnResults

Then chain as manyincludeOnRelationscalls as you like. Each method call should include a searchable Atlan field you want to include on eachrelationshipincluded in each search result. (In this example, every related column that comes back for each search result will include the column's name.)
...
frompyatlan.client.atlanimportAtlanClientfrompyatlan.model.assetsimportTable,Columnfrompyatlan.model.fluent_searchimportFluentSearchclient=AtlanClient()request=(FluentSearch.select()# (1).include_on_results(Table.COLUMNS)# (2).include_on_relations(Column.NAME)# (3)).to_request()# (4)results=client.asset.search(criteria=request)
...
Chain as manyinclude_on_resultscalls as you like, to include one or more relationships on each search result.

include_on_results

Then chain as manyinclude_on_relationscalls as you like. Each method call should include a searchable Atlan field you want to include on eachrelationshipincluded in each search result. (In this example, every related column that comes back for each search result will include the column's name.)
...
{"dsl":{// (1)"query":{...},"from":0,"size":100,"track_total_hits":true},"attributes":[// (2)"columns"],"relationAttributes":[// (3)"name"]"suppressLogs":true,"showSearchScore":false,"excludeMeanings":false,"excludeClassifications":false}

{"dsl":{// (1)"query":{...},"from":0,"size":100,"track_total_hits":true},"attributes":[// (2)"columns"],"relationAttributes":[// (3)"name"]"suppressLogs":true,"showSearchScore":false,"excludeMeanings":false,"excludeClassifications":false}

Define the search query as you normally would.

Specify the list of attributes (relationships) you want to include on each result in theattributeslist of the request.

attributes

Specify the list of attributes you want to include on eachrelationshipin each search result in therelationAttributeslist of the request.
