...
(source: https://developer.atlan.com/snippets/access/events/)
...
Access controlAccess controlManage personasManage purposesManage policiesAccess eventsAccess eventsTable of contentsAll eventsAdmin eventsAPI token managementRun queries on an asset
...
Access eventsAccess eventsTable of contentsAll eventsAdmin events
...
... access thelogsendpoints.

logs

ThegetEvents()method allows you to filter across all access events that are logged.

getEvents()

(Optional) You can filter by events only back to a particular point in time (using the formatyyyy-MM-dd).

yyyy-MM-dd

(Optional) You can filter by events only up to a particular point in time (using the formatyyyy-MM-dd).
...
Begin by defining your filter criteria in aKeycloakEventRequest.
...
From a client, call theadmin.get_keycloak_events()method with your requested filters.
...
ThegetAdminEvents()method allows you to filter across all admin events that are logged.
...
Begin by defining your filter criteria in aAdminEventRequest.
...
From a client, call theadmin.get_admin_events()method with your requested filters.
