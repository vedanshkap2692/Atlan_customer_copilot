# Events when custom metadata is added to an asset¶
(source: https://developer.atlan.com/events/scenarios/custom-metadata-add/)
...
EventsEventsEvent triggersEvent triggersAsset is createdAsset is updatedAsset is deletedCustom metadata is addedCustom metadata is addedTable of contentsBUSINESS_ATTRIBUTE_UPDATECustom metadata is removedAsset is taggedAsset is untaggedLineage is createdEvent typesEvent typesENTITY_CREATEENTITY_UPDATEENTITY_DELETEBUSINESS_ATTRIBUTE_UPDATECLASSIFICATION_ADDCLASSIFICATION_DELETE

Event triggersEvent triggersAsset is createdAsset is updatedAsset is deletedCustom metadata is addedCustom metadata is addedTable of contentsBUSINESS_ATTRIBUTE_UPDATECustom metadata is removedAsset is taggedAsset is untaggedLineage is created
...
Custom metadata is addedCustom metadata is addedTable of contentsBUSINESS_ATTRIBUTE_UPDATE
...
When custom metadata is added to anassetin Atlan, the following event is emitted.
...
Atlan always emits aBUSINESS_ATTRIBUTE_UPDATEevent when custom metadata is added to an asset.
